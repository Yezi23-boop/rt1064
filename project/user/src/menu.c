...
#define HOME_ENCODER_REFRESH_MS (100u)  // Home 页编码器显示刷新间隔，避免主循环高频整屏重画。
...
static uint32 home_encoder_last_ms;     // 上一次 Home 编码器显示刷新时间，单位 ms。
...
static void refresh_home_encoder_display(void)
{
    ...
    now_ms = time_ms();
    if((now_ms - home_encoder_last_ms) >= HOME_ENCODER_REFRESH_MS)
    {
        home_encoder_last_ms = now_ms;
        mark_redraw();
    }
}
...
static void draw_home_page(void)
{
    ...
    screen_draw_home(home_item_names, HOME_ITEM_COUNT, home_cursor, current_map, run_mode, settings_get_save_state(), status->wheel_feedback_count);
}
...
void menu_init(void)
{
    ...
    home_encoder_last_ms = key_last_scan_ms;
    demo_active = 0;
    demo_visible = 0;
    run_state = "Idle";
    clear_result(&last_result);
