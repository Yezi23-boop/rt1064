...
- 规则来源：第21届智能车竞赛人工智能视觉组比赛细则（卓晴博客，文章号 `154697884`）。本项目按“虚实结合”理解任务。
- 真实车模在实体场地中运动，比赛系统根据车顶标识牌定位真实车模的位置和方向，并映射到虚拟游戏场景。
- 箱子是否能被推动、箱子位置变化、目标点消失等逻辑只发生在 `solver.c` 的虚拟推箱子系统中。
- 实车不需要真实接触或推动物理箱子，但必须按照虚拟推箱子求解得到的路径在真实场地中准确行驶，使虚拟车模完成推箱子动作。
- 真实车模仍需遵守虚拟墙/障碍约束；真实位置穿过虚拟墙会导致虚拟系统不再正确刷新坐标，严重时比赛结束。
- 调试运动控制时，不要把“推箱子不准”优先理解为物理推箱失败；应先检查路径执行、格点定位、里程计/yaw、速度和到点判定。
- `U/D/L/R` 仍表示虚拟系统中的推箱动作边界，用于保留路径语义和显示/统计；底盘执行层只把它当作对应方向的移动路径点。
